...
/*   Updated: 2020/10/25 12:02:53 by artainmo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
**Second thread for each philosopher to constantly check if one is dying
...
static void		*dead_check(void *arg)
{
	...
	while (1)
	{
				...
				&& p->p->number_of_times_each_philosopher_must_eat * p->p->number_of_philosophers <= g_eating_counter)
		{
			...
			if (g_eat_count == 0)
				write(1, "number of times philosopher must eat attained\n", 52);
			g_eat_count = 1;
